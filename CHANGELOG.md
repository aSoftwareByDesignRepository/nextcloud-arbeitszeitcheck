...
### Added
- **Offline stamp queue (companion + kiosk):** Idempotent clock/break replay with `clientRequestId` + `occurredAt`; tables `at_mob_stamp_idem` / `at_kiosk_stamp_idem`; capability `mobile.offlineStampQueue`; kiosk `POST /api/kiosk/stamp` for NFC replay without one-shot session.

### Security
- Claim-before-mutate idempotency so concurrent retries cannot double-apply a stamp.
...
### Added
- **Time picker minute step (admin):** every minute by default; optional 5/10/15 under Hours & rest. Fixes inability to book times like 11:51 (#34).
- **Sunday work on working-time models:** optional “Sunday work permitted” flag (also honored when weekday schedule marks Sunday as a work day). Suppresses Sunday compliance warnings for that model only; holiday checks unchanged (#35).
- **Hours display preference:** employees can choose decimal (5.5, legacy default) or hours+minutes (5h 30) under My settings → Breaks (#36).

### Changed
- Default time-picker step is **1 minute** (legacy data unchanged; admins can restore 5-minute steps).
